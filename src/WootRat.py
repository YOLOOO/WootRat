...
def run_woot_rat(sensitivity=None, deadzone=None, curve_factor=None, mouse_active=True, key_mapping=None):
    ...
    while mouse_active:
        ...
        if value_up > deadzone:
            dy -= process_input(value_up, deadzone, curve_factor) * sensitivity
        if value_down > deadzone:
            dy += process_input(value_down, deadzone, curve_factor) * sensitivity
        if value_left > deadzone:
            dx -= process_input(value_left, deadzone, curve_factor) * sensitivity
        if value_right > deadzone:
            dx += process_input(value_right, deadzone, curve_factor) * sensitivity
